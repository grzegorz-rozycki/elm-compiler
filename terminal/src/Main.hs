  ...
  let
    ...
    makeFlags =
      flags Make.Flags
        ...
        |-- flag "output" Make.output "Specify the name of the resulting JS file. For example --output=assets/elm.js to generate the JS at assets/elm.js or --output=ast.json to export the AST in JSON format or --output=/dev/null to generate no output at all!"
